...
class VirtualCladeCollectionManager():
    ...
    def _create_cc_info_dict(self):
        ...
        for n in range(self.obj_manager.sp_data_analysis.parent.args.num_proc):
...
class VirtualCladeCollection:
    ...
    def __init__(
            self, cc_manager, clade, footprint_as_frozen_set_of_ref_seq_uids, ref_seq_id_to_rel_abund_dict,
            total_seq_abundance, cc_object, above_cutoff_ref_seqs_obj_set, ordered_dsss_objs):
...
class VirtualAnalysisType():
    ...
    def __init__(self, at_manager, clade_collection_obj_list, ref_seq_obj_list, id):

        self.id = id
        self.manager = at_manager
        ...
        self.clade = clade_collection_obj_list[0].clade
        ...
        self.artefact_ref_seq_uids_set = None
        self.non_artefact_ref_seq_uids_set = None
        self.basal_seqs_set = None
        self.co_dominant = None
        self.majority_reference_sequence_uid_set = None
        self.name = None

        # key = ref seq id, val=RefSeqReqAbund object
        self.prof_assignment_required_rel_abund_dict = {}

        self._init_virtual_analysis_type()

    def _check_is_list_or_set_else_warn(self, object):
        if isinstance(object, list) or isinstance(object, set):
            return True
        else:
            raise TypeError('clade_collection_objs_to_add must be either a list or a set.')

    def add_ccs_and_reinit_virtual_analysis_type(self, clade_collection_objs_to_add):
        self._check_is_list_or_set_else_warn(clade_collection_objs_to_add)
        self.clade_collection_obj_set_profile_discovery = self.clade_collection_obj_set_profile_discovery.union(set(clade_collection_objs_to_add))
        self._init_virtual_analysis_type()

    def remove_cc_and_reinit_virtual_analysis_type(self, clade_collection_objs_to_remove):
        self._check_is_list_or_set_else_warn(clade_collection_objs_to_remove)
        self.clade_collection_obj_set_profile_discovery = self.clade_collection_obj_set_profile_discovery - set(clade_collection_objs_to_remove)
        self._init_virtual_analysis_type()

    def reinit_virtual_analysis_type(self, new_clade_collection_obj_set):
        self._check_is_list_or_set_else_warn(new_clade_collection_obj_set)
        self.clade_collection_obj_set_profile_discovery = set(new_clade_collection_obj_set)
        self._init_virtual_analysis_type()

    def _init_virtual_analysis_type(self):
        at_df = self._make_rel_abund_dfs()

        self._generate_maj_ref_seq_set_and_infer_codom(at_df)

        self._populate_max_min_dict_and_artefact_set()

        self.non_artefact_ref_seq_uids_set = [
            rs_id for rs_id in self.ref_seq_uids_set if rs_id not in self.artefact_ref_seq_uids_set]
    ...
    def _populate_max_min_dict_and_artefact_set(self):
        ...
        for rs_col in self.relative_seq_abund_profile_assignment_df:
            max = self.relative_seq_abund_profile_assignment_df[rs_col].max()
            min = self.relative_seq_abund_profile_assignment_df[rs_col].min()
            if min < 0.06:
                min = 0.0001
                self.artefact_ref_seq_uids_set.add(rs_col)
            self.prof_assignment_required_rel_abund_dict[rs_col] = self.RefSeqReqAbund(
                max_rel_abund=max, min_rel_abund=min)

    def _generate_maj_ref_seq_set_and_infer_codom(self, at_df):
        ...
        self.majority_reference_sequence_uid_set = majority_reference_sequence_uid_set
        if len(self.majority_reference_sequence_uid_set) > 1:
    ...
    def _reorder_dfs(self, at_df, prof_ass_df):
        ...
        self.relative_seq_abund_profile_discovery_df = at_df.reindex(
            prof_ass_df.sum().sort_values(ascending=False).index, axis=1)
        self.relative_seq_abund_profile_assignment_df = prof_ass_df.reindex(
            prof_ass_df.sum().sort_values(ascending=False).index, axis=1)
    ...
    def _create_rel_seq_abund_prof_assign_df(self, at_df):
        ...
        prof_ass_df = prof_ass_df[:, 0:-1].div(self.relative_seq_abund_profile_assignment_df["sum"], axis=0)
        return prof_ass_df

    def _create_rel_seq_abund_profile_disco_df(self):
        # create and populate the relative_seq_abund_profile_discovery_df
        at_df = pd.DataFrame(index=[cc.id for cc in self.clade_collection_obj_set_profile_discovery],
                             columns=[rs.id for rs in self.footprint_as_ref_seq_objs_set])
        for cc in self.clade_collection_obj_set_profile_discovery:
            ref_seq_abund_dict_for_cc = self.manager.obj_manager.cc_manager.clade_collection_instances_dict[
                cc.id].ref_seq_id_to_rel_abund_dict
            at_df.loc[cc.id] = pd.Series(
                {rs_uid_key: rs_rel_abund_val for rs_uid_key, rs_rel_abund_val in ref_seq_abund_dict_for_cc.items() if
    ...
    def _set_basal_seq(self):
        basal_set = set()
        found_c15_a = False
        for rs in self.footprint_as_ref_seq_objs_set:
            if rs.name == 'C3':
                basal_set.add('C3')
            elif rs.name == 'C1':
                basal_set.add('C1')
    ...
    def _generate_name(self):
        if self.co_dominant:
            list_of_maj_ref_seq = [rs for rs in self.footprint_as_ref_seq_objs_set if rs.id in self.majority_reference_sequence_uid_set]
            ...
            for ref_seq_id in list(self.relative_seq_abund_profile_assignment_df):
